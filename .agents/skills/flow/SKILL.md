...
7. Run each stage from the entry point through `pr` in this order, honouring each confirmation as it is reached rather than afterwards: `$issue` → confirm the issue → `$design` → confirm the design → `$implement` → `$review` → triage → `$pr`. Pass the base branch through unchanged. When the run starts at `$issue`, capture the number it returns and use it for every later stage.
8. The two confirmations in step 7: the first covers an issue this run created, and is skipped when the run entered at design or later — a resumed run arrives with a number already. The second runs on resumed runs too, because a design commit proves the artifacts were written, not approved; skip it when entering at `$review` or `$pr` (implementation already happened) or when entering at `$implement` with no design artifacts, since `$implement` treats design as optional.
9. Before `$pr`, triage the review findings. Critical findings are not deferrable and block PR creation until fixed. Major findings may be deferred only after an advisor consult, with the rationale recorded in the review artifact. Minor findings and suggestions are noted, not gated on.
10. Stop at the first stage that cannot complete, and report which stage and why.

## Output

Report the entry stage and why it was chosen, which confirmations ran and which were skipped, the per-stage outcome including stages skipped as already complete, the artifacts produced, the review verdict with the disposition of each Critical and Major finding, the PR URL and CI status, the merge result when merging was requested, and anything left incomplete.
