...
- **Always-latest asset refresh** (#279): A complementary distribution mode for high-update-frequency operational assets — `.claude/commands/`, `.claude/skills/`, `.claude/scripts/`, `.claude/rules/` (language-agnostic only; language-specific rules continue to ship via language plugins). Manifest-driven upgrade is opt-in and rarely run; for assets that are intended to be **identical across all projects**, the copy-once-then-track model is the wrong abstraction. Instead, `templates/core/.devcontainer/scripts/refresh-assets.sh` runs from `postStartCommand` (every container start) and from `post.sh` (first boot). On each run it (a) `git ls-remote`s upstream tarnished's `develop` and compares against the SHA of the cached clone at `/opt/tarnished` (configurable via `clone_dir` in `refresh.json`; falls back to `${HOME}/.cache/tarnished` if `/opt/tarnished` is not writable), (b) when SHAs differ, performs `git fetch` + `git reset --hard origin/<branch>` (the cache is treated as an immutable mirror — local edits are never expected), (c) `rsync --delete`s each whitelisted path from the cache into the project, then (d) overlays user-owned `.local/` sidecars (`.claude/commands.local/`, `.claude/skills.local/`, `.claude/scripts.local/`, `.claude/rules.local/`) by re-rsyncing without `--delete`. The whitelist is declared in `<target>/.tarnished/refresh.json` (schema documented in `data-model.md` and `api-spec.md`). The script always returns 0 — every failure path (clone fail, ls-remote fail, fetch fail, rsync fail, malformed config, missing config) emits `print_warning` and exits 0 so container start is never blocked (FR-5). Mutual exclusivity with #265's manifest tracking is enforced by extending `MANIFEST_EXCLUDE_GLOBS` (in `scripts/lib/common.sh`) with both the always-latest paths and their `.local/` sidecars; this keeps `--create-manifest` from hashing them and keeps `--upgrade` from recording or applying decisions to them. The whitelist in `refresh.json` is the runtime contract; the `MANIFEST_EXCLUDE_GLOBS` extension is the build-time contract — they are kept in sync by code review and a unit test that asserts `refresh.json`'s defaults are a subset of `MANIFEST_EXCLUDE_GLOBS`. Per-issue design lives in `docs/design/#279/design.md`. The workspace itself dogfoods the same setup (mirroring the `setup_codex` convention from #261): `/workspace/.devcontainer/scripts/refresh-assets.sh`, `/workspace/.tarnished/refresh.json`, and the `postStartCommand` wiring in `/workspace/.devcontainer/devcontainer.json` are kept byte-identical (modulo absolute paths) with the templates.
