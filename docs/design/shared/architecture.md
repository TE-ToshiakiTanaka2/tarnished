...
│   │   │   ├── agents/               # Subagent definitions (code-reviewer, advisor #308); always-latest (#279)
...
│   │                                 #   upstream tarnished into /opt/tarnished and rsyncs whitelisted
...
- **Role-based delegation policy** (#308): Skills name **roles**, never models. Four roles — `orchestrator` (judgment), `executor` (mechanical/high-volume), `external-reviewer` (independent cross-vendor review), `advisor` (read-only second opinion) — are routed by *nature of work* rather than by lifecycle stage, so a single stage can be part-inline and part-delegated. The routing table lives once in `.claude/skills/_shared/delegation/SKILL.md`, which sits in the refresh-managed channel so it updates in lockstep with the skills that consume it. Role→agent/model binding lives in `.tarnished/agent-profile.json :: roles`, which is manifest-tracked rather than refresh-managed so a downstream retarget survives every container start. Escalation is replaced by up-front routing: retry budgets survive only for mechanical steps and only for command-level failure ("the command errored"), never for "the model misjudged". The `advisor` role is capability-gated for primary agents (e.g. the `codex-main` profile) that have no subagent mechanism; where it is unavailable the consult is skipped, recorded, and never blocks a stage.
- **Advisor as standing content review** (#312): the advisor's consults are split into two classes and every invocation point is labelled with one. `challenge` runs *before* an artifact is written and asks whether the intended choice is right; `conformance` runs *after* and asks whether the artifact carries the requirement it was supposed to carry. Until #312 only the first class existed, and the second question was answered by two human approval gates in `/flow` — which is why removing those gates required staffing the question rather than deleting it. Consults are unconditional rather than judgment-triggered ("consult when the result looks doubtful" is the failure mode being removed, not a lighter version of it), the advisor proposes concrete fixes rather than objections alone, and it reviews the branch diff in parallel with the `external-reviewer`. It stays read-only throughout: it labels findings blocking or noting, and the orchestrator decides, applies, and owns the outcome. Two properties keep gate removal honest — `docs/design/#{issue}/conformance.md` records every verdict durably (including skips) so `/flow`'s evidence derivation can tell a verified design commit from an unverified one, and `/review` gains the issue's Requirements as a second ground truth so a requirement dropped upstream of the design is still catchable. Every role is also bound to an explicit model for the first time (#312): until then no level of any binding chain named a model, so all four roles ran on the session's, and the "second opinion" was independent in context but not in weights. Each role is now pinned in exactly one place, chosen by how it is dispatched — `orchestrator` in `.claude/settings.json` (it is the session, so no subagent channel reaches it), `executor` and `advisor` in their `.claude/agents/*.md` frontmatter, and `external-reviewer` in `.codex/config.toml`, whose keys were removed from `roles.external-reviewer` so the reviewer's model has one declaration site rather than two. #312 also adds the missing `executor.md` definition, a role that had a binding entry since #308 but no agent file to carry either its model or its purpose. The cost is stated rather than implicit: worst case 11 advisor invocations per run on a separately-priced model, against ≤1 inherited invocation before.
...
- **Always-latest asset refresh** (#279, #281): A complementary distribution mode for high-update-frequency operational assets — `.claude/commands/`, `.claude/skills/`, `.claude/scripts/`, and file-managed shared rules such as `.claude/rules/shell.md` (language-specific rules continue to ship via language plugins). Manifest-driven upgrade is opt-in and rarely run; for assets that are intended to be **identical across all projects**, the copy-once-then-track model is the wrong abstraction. Instead, `templates/core/.devcontainer/scripts/refresh-assets.sh` runs from `postStartCommand` (every container start) and from `post.sh` (first boot). On each run it (a) `git ls-remote`s upstream tarnished's `develop` and compares against the SHA of the cached clone at `/opt/tarnished` (configurable via `clone_dir` in `refresh.json`; falls back to `${HOME}/.cache/tarnished` if `/opt/tarnished` is not writable), (b) when SHAs differ, performs `git fetch` + `git reset --hard origin/<branch>` (the cache is treated as an immutable mirror — local edits are never expected), (c) rsyncs each whitelisted path from the cache into the project, using `--delete` only for directory-managed paths, then (d) overlays user-owned `.local/` sidecars by re-rsyncing without `--delete` for directories. The whitelist is declared in `<target>/.tarnished/refresh.json` (schema documented in `data-model.md` and `api-spec.md`). The script always returns 0 — every failure path (clone fail, ls-remote fail, fetch fail, rsync fail, malformed config, missing config) emits `print_warning` and exits 0 so container start is never blocked (FR-5). Mutual exclusivity with #265's manifest tracking is enforced by extending `MANIFEST_EXCLUDE_GLOBS` (in `scripts/lib/common.sh`) with the always-latest paths and their `.local/` sidecars; this keeps `--create-manifest` from hashing them and keeps `--upgrade` from recording or applying decisions to them. The whitelist in `refresh.json` is the runtime contract; the `MANIFEST_EXCLUDE_GLOBS` extension is the build-time contract — they are kept in sync by code review and a unit test that asserts `refresh.json`'s defaults are a subset of `MANIFEST_EXCLUDE_GLOBS`. Per-issue design lives in `docs/design/#279/design.md`; #281 fixes the canonical source to `templates/claude/.claude` and protects language-specific rule files from directory `--delete`. The workspace itself dogfoods the same setup (mirroring the `setup_codex` convention from #261): `/workspace/.devcontainer/scripts/refresh-assets.sh`, `/workspace/.tarnished/refresh.json`, and the `postStartCommand` wiring in `/workspace/.devcontainer/devcontainer.json` are kept byte-identical (modulo absolute paths) with the templates.
