...
For trusted files, update state only after successful installation or a verified current==desired match. Retain the old baseline on conflict/failure, user deletion, or an upstream removal not pruned. Keep removed-file baselines so a later `--prune` can still make a safe decision. A missing eligible v1 path becomes a separate `deleted_paths` tombstone, never a trusted hash. Preserve that deletion intent across upgrade/bootstrap/refresh until the developer restores exact distribution bytes for adoption. A successfully pruned file loses its entry. Never advance skipped files to desired upstream hashes.

## Per-file refresh and provenance

Persist `.tarnished/refresh-state.json` atomically. Its versioned entries bind a project-relative destination to the upstream repository and source mapping, the last installed SHA-256, and whether effective bytes came from upstream or a local overlay. Include the upstream commit for diagnostics. Resolve explicitly selected project/source/staging/cache roots once to their physical locations using portable directory traversal. A symlink root leaf is rejected, while symlinked host ancestors (such as a host `/var` alias) may be canonicalized. Configuration/state files and every path component below those physical roots must remain ordinary files/directories, not symlinks. State is generated local metadata, excluded from both manifests and distribution; document ignore handling without rewriting user gitignore content.
...
The default catalog contains existing Claude mappings, Codex `.agents/skills` with `.agents/skills.local`, shared workflow `.md` file mappings with `.tarnished/workflows.local/<file>`, and the existing separate erd directory mapping. Shared contracts are profile-neutral and read current choices from project-owned `agent-profile.json`, so fresh scaffold bytes can be adopted by refresh. Older rendered contracts remain conservative conflicts requiring one-time comparison/adoption. Using file mappings for shared contracts avoids an overlapping parent-directory mapping and its generated erd projection. Add every default destination/overlay and required directory-plus-`/*` exclusions to `MANIFEST_EXCLUDE_GLOBS`. Exclude profile/config/state explicitly; none is fetched as a workflow.
...
Conflict recovery is manual and concrete: compare the reported current path to the cached/source candidate. To retain edits, save them in the corresponding `.local` sidecar and explicitly copy those effective overlay bytes to the live destination before rerunning. To choose upstream alone, remove or move aside the corresponding overlay and explicitly copy upstream bytes to the live destination. Rerunning adopts an exact effective desired match; an existing baseline does not authorize replacing a different chosen overlay merely because the live file matches upstream. Unknown missing-state legacy files cannot automatically receive a differing update; report this limitation. Existing custom whitelists require one-time intentional catalog adoption, not recurring skill optimization.

## Error handling and boundaries

Reject absolute/empty/dot/traversal paths, control characters unsupported by the serialization, directory/file collisions, overlapping destination mappings, and symlink components or leaf files in source, target, overlay, state, manifest, and module scope. Only enumerate regular files; never copy symlinks from upstream or sidecars. Recheck expected current hashes immediately before replacement/deletion, verify copied temporary content, and verify the installed ordinary-file hash after portable replacement before advancing state. Recursive parent cleanup must stop at the managed root; omitting empty-directory cleanup is acceptable and simpler. Never recursively delete destination directories.
