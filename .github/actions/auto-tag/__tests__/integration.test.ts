...
import { getDefaultConfig, loadConfig, validateConfig } from "../src/config-loader.js";
import { postFailureComment, postSuccessComment } from "../src/pr-commenter.js";
...
describe("Integration: Auto Tag Flow", () => {
  ...
  describe("Full workflow: PR merged -> Tag created", () => {
    it("should create patch version tag for feature branch", async () => {
      ...
      const currentVersion = parseVersion(latestTag ?? "v0.0.0");
...
`;
  ...
  describe("Error scenarios", () => {
    it("should handle tag already exists error", async () => {
      mockOctokit.rest.git.createRef.mockRejectedValueOnce(new Error("Reference already exists"));
    ...
    it("should handle generic tag creation error", async () => {
      mockOctokit.rest.git.createRef.mockRejectedValueOnce(new Error("Permission denied"));
