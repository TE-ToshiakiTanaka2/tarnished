...
import { GET_PROJECT_ITEMS, GET_ISSUE_BY_NUMBER } from '../graphql/queries.js';
...
export async function addItemToProject(client, projectId, contentId) {
    ...
    const response = await client.mutate(ADD_PROJECT_ITEM, { projectId, contentId });
...
export async function getIssueNodeId(client, owner, repo, issueNumber) {
    ...
    try {
        const response = await client.query(GET_ISSUE_BY_NUMBER, { owner, repo, number: issueNumber });
