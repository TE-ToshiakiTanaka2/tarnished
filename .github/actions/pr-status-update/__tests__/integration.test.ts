...
import { addItemToProject, findItemInProject, getIssueNodeId } from "../src/project/item.js";
import type { ExtendedProjectConfig, FieldInfo } from "../src/types.js";
...
describe("Integration: PR Status Update Flow", () => {
  ...
  describe("Error handling in workflow", () => {
    ...
    it("should handle GraphQL API errors gracefully", async () => {
      ...
      await expect(
        getIssueNodeId(mockClient as unknown as GraphQLClient, "test-owner", "test-repo", 123),
  ...
  describe("Pagination handling", () => {
    it("should handle paginated project items", async () => {
      ...
      mockClient.query.mockResolvedValueOnce({
        node: {
          items: {
            nodes: [{ id: "ITEM_TARGET", content: { id: targetIssueId } }],
            pageInfo: { hasNextPage: false, endCursor: null },
          },
        },
      });
