 ...
 */
export function loadConfig(configPath: string): ExtendedProjectConfig {
 ...
 */
export function validateConfig(config: unknown): ExtendedProjectConfig {
  ...
  const pr = cfg.pr;
  let prConfig: ExtendedProjectConfig["pr"] | undefined;
  ...
  if (pr !== undefined) {
    ...
    prConfig = {
      status: prObj.status as string,
      branch_pattern: prObj.branch_pattern as string | undefined,
    };
  }
