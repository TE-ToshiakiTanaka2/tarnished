/**
 * Base configuration file schema for project-automation
 */
...
/**
 * Extended configuration with PR-specific settings
 */
export interface ExtendedProjectConfig extends ProjectConfig {
  pr?: {
    /** Status value to set when PR is opened/reopened */
    status: string;
    /** Optional regex pattern for extracting issue number from branch name */
...
export interface DetectedIssues {
  /** Issues detected from keywords (Closes #, Fixes #, etc.) */
  fromKeywords: number[];
  /** Issues detected from branch name */
  fromBranch: number[];
  /** All unique issues (deduplicated) */
  all: number[];
}
...
/**
 * GraphQL response for getting issue by number
 */
