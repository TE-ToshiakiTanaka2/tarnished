...
Tarnished maintains distributed AI skills, workflows and complete `.claude/settings.json` / applicable `.codex/config.toml` files. Bare setup reruns and `--refresh` update AI assets; root-inclusive `--upgrade` also updates eligible runtime helpers, while module-only upgrades retain their scope. Currently distributed AI targets are installed even when edited, unknown or previously deleted; differing existing bytes require a verified backup before replacement, and matching bytes need no backup. Custom keys inside distributed settings files are backed up and replaced using the selected distribution values. Preserve application code, scaffold seeds, project-only siblings, profile/model overrides in `.tarnished/agent-profile.json`, native local settings and configured `.local` sidecars. Native local settings are not whole-file replacement overlays. Backups use unique private runs under `.tarnished/backups/`, retain original relative paths, are excluded from refresh/prune ownership and are never automatically deleted. Report restoration paths; failed backups or detected concurrent changes preserve the live file and prior baseline. Only proven unchanged upstream-origin files may be removed after verified upstream disappearance; never infer ownership by walking project files. Use current setup for legacy catalog and installed-updater migration before relying on container-start refresh. Unrecognized installed updaters require explicit review and migration. A root AI update may proceed when helper work is blocked, but the combined upgrade must report helper failures with a nonzero result.
