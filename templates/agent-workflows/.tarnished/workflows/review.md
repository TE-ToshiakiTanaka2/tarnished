...
4. Resolve the reviewer in priority order:
   1. The agent bound to the `external-reviewer` role in `.tarnished/agent-profile.json`, defaulting to `{{AI_REVIEW_AGENT}}`, when it is installed and distinct from the primary agent.
...
5. Review for bugs and logic errors, security, performance, code quality, type safety, error handling, test coverage, and adherence to the design artifacts. The canonical criteria list, worded for a reviewer prompt, is the "Review Criteria" section of the Review Prompt Template in `.claude/skills/review/SKILL.md`; insert it inline when constructing the prompt rather than referring the reviewer to a path.
...
7. Classify each finding as Critical, Major, Minor, or Suggestion. Fix Critical and Major before the pull request; record a rationale for anything deliberately deferred. Then append a fix summary to the review artifact.
