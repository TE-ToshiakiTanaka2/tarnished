...
3. Determine inspection depth from diff size and affected areas; assess all nine criteria even for small changes. Parse reviewer flags separately from the optional target branch.
4. Resolve the reviewer in priority order:
   1. The agent bound to the `external-reviewer` role in `.tarnished/agent-profile.json`, defaulting to its `review_agent` field (treat absent or unrendered values as unavailable), when it is installed and distinct from the primary agent. Its model and reasoning effort come from the reviewer CLI's own config, which is their single source.
...
7. For review-only requests, report findings without applying fixes. When fixes are requested or review is part of `flow`, classify each finding as Critical, Major, Minor, or Suggestion — triage is the orchestrator's. Fix Critical and Major before the pull request, delegating the application to the `executor`; record a rationale for anything deliberately deferred. Then append verified fix dispositions and completion evidence, using `.claude/skills/review/references/completion.md` when available. Completion must cover the current implementation, fetched target commit, and issue body; a clean review may record `None required` without a fix commit. A report-only review with unresolved required fixes cannot satisfy completion.
