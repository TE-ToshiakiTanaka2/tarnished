...
setup_mcp() {
    ...
    if claude mcp list 2>/dev/null | grep -q "serena"; then
        ...
        if command -v uvx &> /dev/null; then
            ...
            claude mcp add serena -- uvx --from "git+https://github.com/oraios/serena" serena start-mcp-server --context ide-assistant --enable-web-dashboard false --enable-gui-log-window false
