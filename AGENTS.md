...
Claude assets, applicable Codex `.agents/skills`, shared workflow contracts and the Codex-readable erd projection are refreshed per file from upstream Tarnished by `refresh-assets.sh` on every container start (`postStartCommand`) and on first boot (via `post.sh`). When reviewing changes that touch this surface, verify:
...
- Refresh must preserve project-added files, locally edited/deleted assets, and `.local` sidecars. Recommend configured sidecars for explicit overrides; a live-file edit is a preserved conflict. Only proven unchanged upstream-origin files may be removed after verified upstream disappearance. Never use recursive destination deletion or infer ownership by walking project files.
