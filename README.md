...
Set up a DevContainer environment with shared Claude Code / Codex workflows. The shell setup and maintenance commands require Bash 4.4 or newer, Git, jq, and either `sha256sum` or `shasum`. On macOS, use an installed modern Bash; the stock Bash 3.2 is unsupported. Maintenance supports ordinary BSD utilities and does not require GNU `realpath` or `mv -T`:
...
The live projection updates only when it matches its previous installed baseline. Direct edits to the live projection are preserved as conflicts. Removing a sidecar restores upstream only if the projection remains unchanged; overlay-origin files absent from both upstream and sidecar are preserved. Overlay-only added files are supported. For shared erd behavior, duplicate overrides into `.tarnished/workflows/erd.local/`; the two erd projections have separate sidecars. Codex uses `.agents/skills.local`; shared contracts use `.tarnished/workflows.local/<file>.md`. The shipped Codex whitelist keeps `.agents/skills.local` local and ignored. To share those overrides, explicitly append `!.agents/skills.local/` and `!.agents/skills.local/**` to your project `.gitignore` after its existing Codex block; existing block markers are unchanged.

For existing installations, inspect `--refresh --dry-run`, then run `--refresh`. The same command replaces a missing, exactly recognized shipped legacy, or unchanged helper with a trusted version-2 installed baseline with the current safe helper. It records that baseline for subsequent updater releases. An edited/unrecognized helper is preserved and its exact source and destination paths are reported: **review and explicitly copy the safe helper before the next container start**. A safe one-shot refresh does not make an unconverted legacy installed script safe. Custom hooks and settings are retained. A missing helper with a recorded baseline or deletion intent stays absent. Creating a previously missing helper only installs the file: inspect `post.sh` and `devcontainer.json` and configure its invocation if needed; setup maintenance does not establish container-start wiring automatically.

When a conflict is reported, compare the named project file with the named source/cache candidate. To retain an override, save the chosen bytes in its corresponding `.local` path, then explicitly copy those same effective overlay bytes to the live destination and rerun. To adopt upstream alone, remove or move aside any corresponding sidecar, copy the chosen upstream bytes into the live destination and rerun. Matching the effective desired content permits safe adoption even when prior state exists; copying upstream into the live destination while retaining a different sidecar does not resolve the conflict. Unknown legacy content that differs from today's distribution needs this one-time review; it is never silently declared owned. Older rendered workflow contracts may conflict with the current profile-neutral contracts: compare and adopt the current file once, keeping profile/model choices in `.tarnished/agent-profile.json`. Files already deleted by older scripts cannot be recovered by provenance migration.

Dry-run writes no target/config/state/summary files or persistent cache, suppresses Git index refresh writes, and uses a valid local source/cache for exact planned actions. If none exists, it reports that the upstream comparison is unavailable. Refresh state is generated local metadata: add `.tarnished/refresh-state.json` to your ignore rules if it is not already ignored; maintenance does not rewrite your `.gitignore`.
